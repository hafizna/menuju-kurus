    ...
    <div className="space-y-4 p-4">
      ...
      <section className="rounded-2xl bg-white p-4 shadow-sm dark:bg-neutral-900">
        ...
        <div className="mt-3 grid grid-cols-3 gap-2 text-center">
          <div><div className="text-lg font-semibold">{summary.caloriesIn}</div><div className="text-xs text-neutral-500">Masuk</div></div>
          <div><div className="text-lg font-semibold">{summary.caloriesOut}</div><div className="text-xs text-neutral-500">Terbakar</div></div>
          <div><div className={`text-lg font-semibold ${over ? "text-red-500" : ""}`}>{summary.remaining >= 0 ? summary.remaining : 0}</div><div className="text-xs text-neutral-500">Sisa budget</div></div>
        </div>
      ...
      <Link href="/log-food" className="block rounded-xl border-2 border-dashed border-brand-400 px-4 py-4 text-center font-medium text-brand-600 dark:text-brand-400">
            ...
            .map((entry) => (
              <div key={entry.id} className="flex items-center justify-between rounded-xl bg-white px-4 py-3 shadow-sm dark:bg-neutral-900">
                <div>
                  ...
                  <div className="text-xs text-neutral-500">{new Date(entry.time).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" })}</div>
                </div>
                <div className="flex items-center gap-3">
                  <span className={entry.kind === "meal" ? "text-sm font-medium" : "text-sm font-medium text-brand-600"}>{entry.kind === "meal" ? "+" : "-"}{entry.calories} kcal</span>
                  <button onClick={() => removeEntry(entry.kind, entry.id)} className="text-neutral-300 hover:text-red-400" aria-label="Hapus">✕</button>
                </div>
              </div>
            ))}
        </section>
